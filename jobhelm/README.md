...
- **Apply queue** — tick roles in Discover, press **Prepare N applications**, and JobHelm adds each to your board and writes a reviewable pack: the apply URL and detected ATS, your application fields, drafted screening answers, and a pre-submit checklist. Then the **Apply** tab walks you through them one at a time — open the posting, click the autofill bookmarklet, check the answers, attach the résumé, submit. **JobHelm never submits for you**: the bookmarklet runs in your own tab, fills only identity fields, and tells you which required fields it deliberately left alone (work authorization, sponsorship, salary — answers that are yours to make). Nothing here needs your portal credentials or the automation that gets job-board accounts restricted.
